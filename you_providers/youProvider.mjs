...
class YouProvider {
    constructor(config) {
        this.config = config;
        this.sessions = {};
        this.browserInstances = new Map(); // 用于跟踪每个 cookie 的浏览器实例
        this.pagePool = [];
        this.maxPages = 5;
    ...
    async init(config) {
        ...
        if (process.env.USE_MANUAL_LOGIN === "true") {
            ...
            console.log("当前动登录模式，跳过config.mjs文件中的 cookie 验证");
        } else {
            // 使用配置文件中的 cookie
            for (let index = 0; index < config.sessions.length; index++) {
                ...
                if (jwtSession && jwtToken) {
                    ...
                    try {
                        ...
                        // 使用 index 作为唯一标识
                        const sessionId = `${jwt.user.name}_${index}`;
                        ...
                        console.log(`已添加 #${index} ${sessionId} (旧版cookie)`);
                    } catch (e) {
                    ...
                    try {
                        ...
                        // 使用 index 作为唯一标识
                        const sessionId = `${jwt.email}_${index}`;
                        this.sessions[sessionId] = {
                            configIndex: index,
                            ds,
                            dsr,
                        ...
                        console.log(`已添加 #${index} ${sessionId} (新版cookie)`);
                        if (!dsr) {
        ...
        for (const [sessionId, session] of Object.entries(this.sessions)) {
            try {
                // 提取基础用户名（不包含索引）
                const baseUsername = sessionId.split('_')[0];
                const configIndex = sessionId.split('_')[1];
                
                console.log(`为会话 ${sessionId} 创建浏览器实例`);
                const response = await connect({
                    headless: "auto",
                    turnstile: true,
                    customConfig: {
                        userDataDir: path.join(__dirname, "browser_profiles", sessionId),
                    ...
                    fingerprintOptions: {
                        ...
                        userAgent: {
                            platform: Math.random() > 0.5 ? 'Win32' : 'Win64',
                            mobile: false
                        },
                        navigator: {
                            language: 'en-US',
                ...
                await page.goto("https://you.com", {timeout: 120000});
                await sleep(5000);
                ...
                try {
                    ...
                    if (json.subscriptions?.length > 0 || allowNonPro) {
                        console.log(`${sessionId} 有效 ${json.subscriptions?.length > 0 ? '(Pro 计划)' : '(非Pro)'}`);
                        session.valid = true;
                        ...
                        if (json.subscriptions?.length > 0) {
                            session.subscriptionInfo = await this.getSubscriptionInfo(page);
                        }
                    } else {
                        console.log(`${sessionId} 无有效订阅`);
                        await this.clearYouCookies(page);
                        await browser.close();
                    }
                } catch (e) {
                    console.log(`${sessionId} 已失效`);
                    await this.clearYouCookies(page);
                    await browser.close();
                }

            } catch (e) {
                console.error(`初始化浏览器失败 (${sessionId})`);
                console.error(e);
            }
        }
            ...
            if (session.valid) {
                ...
                if (session.subscriptionInfo) {
                    ...
                    if (session.subscriptionInfo.cancelAtPeriodEnd) {
                        console.log('  注意: 该订阅已设置为���当前周期束后取消');
                    }
                } else {
        ...
        console.log('初始化完成，页面池状态:', this.pagePool.map(p => ({
            username: p.username,
            busy: p.busy
        })));
    }
    ...
    async getAvailablePage() {
        console.log('当前页面池状态:', this.pagePool.map(p => ({
        ...
        for (let i = 0; i < this.pagePool.length; i++) {
            ...
            if (!pageObj.busy) {
                console.log(`找到空闲页面: ${pageObj.sessionId}`);
                pageObj.busy = true;
                return pageObj;
            }
        }
        
        console.log('没有找到空闲页面，当前页面池大小:', this.pagePool.length);
        throw new Error("所有页面都在忙，请稍后重试");
    }
        ...
        try {
            ...
            if (this.isCustomModeEnabled && this.isRotationEnabled && this.rotationEnabled) {
                ...
                if (modeId === '1') {
                    ...
                    console.log(`注意: 检测到 -modeid:2，强制切换到自定义模`);
                }
            ...
            if (effectiveUseCustomMode) {
                if (!this.config.sessions[pageObj.configIndex].user_chat_mode_id) {
                    this.config.sessions[pageObj.configIndex].user_chat_mode_id = {};
                }

                // 检查是否存在与当前用户名匹配的记录
                let existingUserRecord = Object.keys(this.config.sessions[pageObj.configIndex].user_chat_mode_id).find(key => key === username);

                if (!existingUserRecord) {
                    // 为当前用户创建新的记录
                    this.config.sessions[pageObj.configIndex].user_chat_mode_id[username] = {};
                    // 写回config
                ...
                // 检查是否存在对应模型的记录
                if (!this.config.sessions[pageObj.configIndex].user_chat_mode_id[username][proxyModel]) {
                    // 创建新的user chat mode
                        ...
                        async (proxyModel, proxyModelName) => {
                            return fetch("https://you.com/api/custom_assistants/assistants", {
                                ...
                                body: JSON.stringify({
                                    ...
                                    includeFollowUps: false, // 是否包含后续问题建议
                    ...
                    if (userChatMode.chat_mode_id) {
                        this.config.sessions[pageObj.configIndex].user_chat_mode_id[username][proxyModel] = userChatMode.chat_mode_id;
                        // 写回config
                        fs.writeFileSync("./config.mjs", "export const config = " + JSON.stringify(this.config, null, 4));
                ...
                userChatModeId = this.config.sessions[pageObj.configIndex].user_chat_mode_id[username][proxyModel];
            } else {
            ...
            let responseStarted = false; // 是否经开始接收响应
            ...
            let unusualQueryVolumeTriggered = false; // 否触发了异常请求量提示
            ...
            page.exposeFunction("callback" + traceId, async (event, data) => {
                ...
                switch (event) {
                    ...
                    case "youChatToken":
                        ...
                        // 检查 'unusual query volume'
                        if (processedContent.includes('unusual query volume')) {
                            const warningMessage = "您在 you.com 账号的使用已达上限当前(default/agent)模式已进入冷却期(CD)。请切换模式(default/agent[custom])或耐心等待冷却结束后再继续使用。";
            ...
            async function resendPreviousRequest() {
                try {
                    ...
                    console.error("重新发送请时发生错误:", error);
                    return false;
                }
            }
            ...
            return {
                ...
                cancel: () => {
                    ...
                    pageObj.busy = false; // 释放页面
                }
            };
        } catch (error) {
            pageObj.busy = false; // 确保在错误时释放页面
            throw error;
        }
    }
}
